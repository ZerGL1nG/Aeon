...
public class NeuralEnvironment
{
    ...
    public NeuralEnvironment Clone()
    {
        ...
        foreach (var neuron in Neurons) {
            if (float.IsNaN(neuron.Inputs.Values.FirstOrDefault())) Console.WriteLine("BULLSHIT");
            //var govno = new Dictionary<string, float>();
            newEnvironment.AddNeuron(
                new Neuron(neuron.ActFunc, neuron.Tag, new Dictionary<string, float>(neuron.Inputs)),
                new List<Neuron>());
        }
    ...
    public List<float> Work(INetworkData input)
    {
        ...
        foreach (var neuron in Neurons) neuron.Reset();
        foreach (var (d, inputNeuron) in data.Zip(Input, (i, n) => (i, n))) inputNeuron.GetInput(d);
        foreach (var neuron in Output) neuron.Work(NeuronsIds);
        return GetResults();
    }
    ...
    {
        foreach (var neuron in Neurons) {
            var sq                                                   = MathF.Sqrt(neuron.Inputs.Count);
            foreach (var id in neuron.Inputs.Keys) neuron.Inputs[id] /= sq;
        }
    }

    public void InitIds()
    {
        foreach (var neuron in Neurons) NeuronsIds.Add(neuron.Id, neuron);

...
public class Stats
{
    ...
    public Stats(Dictionary<Stat, double> data) => this.data = new Dictionary<Stat, double>(data);
    ...
    public List<float> OutFloatsActivated() => data.Select(o => StatConverters.Convert(o.Key, (float)o.Value)).ToList();
    ...
    {
        if (data.ContainsKey(stat)) return data[stat];
        throw new ArgumentException($"No stat named {stat} in stats");
    }

    public static Stats Clone(Stats stats) => new(new Dictionary<Stat, double>(stats.data));

    public static int RoundStat(double value, Stat stat) =>
        stat switch {
    ...
    public override string ToString()
    {
        ...
        foreach (var (stat, value) in data) x.Append($"{stat}: {RoundStat(value, stat)}; ");
        return x.ToString();
    }

    public override int GetHashCode() =>
        HashCode.Combine(HashCode.Combine(data[Stat.Health], data[Stat.Attack], data[Stat.Spell]),
    ...
    public static double CalculateShield(double coeff) =>
        Math.Min(shieldConst1*coeff/(1+shieldConst1*Math.Pow(Math.E, shieldConst2*Math.Log(coeff))), maxShield);
}